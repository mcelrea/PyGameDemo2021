...
#sounds
gunSound = pygame.mixer.Sound("275151__bird-man__gun-shot.wav")
music1 = pygame.mixer.Sound("bensound-happyrock.mp3")
...
def create_enemy_wave(wave_number):
    numEnemiesToCreate = wave_number*2
    for i in range(numEnemiesToCreate):
        createEnemyOffScreen()

def createEnemyOffScreen():
    # 1 = top, 2 = right, 3 = bottom, 4 = left
    side = random.randint(1,4)
    if side == 1:
        enemies.append(SimpleEnemy(random.randint(0,1280),
                                   random.randint(-400,-100)))
    elif side == 2:
        enemies.append(SimpleEnemy(random.randint(1350,1650),
                                   random.randint(0,720)))
    elif side == 3:
        enemies.append(SimpleEnemy(random.randint(0,1280),
                                   random.randint(820,1120)))
    else:
        enemies.append(SimpleEnemy(random.randint(-400,-100),
                                   random.randint(0,720)))
...
def checkBulletCollisionWithEnemy():
    for i in range(len(player_bullets)-1, -1, -1): #length of the list to 0
        for j in range(len(enemies)-1, -1, -1): #length of the list to 0
            if player_bullets[i].getCollisionRectangle().colliderect(enemies[j].getCollisionRectangle()):
                del player_bullets[i]
                del enemies[j]
                break #exit, stop checking these two things....yea

def checkForNewWave():
    global currentWave
    if(len(enemies) == 0):
        currentWave+=1
        create_enemy_wave(currentWave)
...
def checkPlayerInput():
    ...
    if pressed[pygame.K_SPACE]:
        if canShoot:
            ...
            canShoot = False
            pygame.mixer.Sound.play(gunSound)
    else: #they are NOT pressing the spacebar
        canShoot = True

def checkForPlayerHit():
    global gameOver
    for i in range(len(enemies)-1, -1, -1): #length of the list to 0
        if p1.getCollisionRectangle().colliderect(enemies[i].getCollisionRectangle()):
            gameOver = True
